...
class LanguagesViewControllerViewModel: Hashable, Equatable, ObservableObject {
    ...
    public func updateCellViewModels() {
        self.downloadedCellViewModels = TMLanguageModels.shared.localModels.compactMap({
            return LanguagesCellViewModel(language: $0, option: .delete) { [weak self] option in
                self?.handleTap()
            }
        })
        self.notDownloadedCellViewModels = TMLanguages.shared.allTranslateLanguages.filter { !TMLanguageModels.shared.localModels.contains($0) }.compactMap({
